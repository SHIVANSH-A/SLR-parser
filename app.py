...
def slr_parser(input_string, parsing_table, grammar):
    ...
    while True:
        if position >= len(tokens):
            moves.append({"error": "Input consumed but parsing not complete", "stack": stack[:], "buffer": tokens[position:]})
            return False, moves
        ...
        if action_key not in parsing_table:
            moves.append({"error": f"No action for state {current_state} and token '{current_token}'", "stack": stack[:], "buffer": tokens[position:]})
            return False, moves

        action = parsing_table[action_key]
        moves.append({"state": current_state, "token": current_token, "action": action, "stack": stack[:], "buffer": tokens[position:]})

        if action == "accept":
            moves.append({"message": "String accepted!", "stack": stack[:], "buffer": tokens[position:]})
            return True, moves
            ...
            if rule_num not in grammar:
                moves.append({"error": f"Invalid rule number {rule_num}", "stack": stack[:], "buffer": tokens[position:]})
                return False, moves
            ...
            if goto_key not in parsing_table:
                moves.append({"error": f"No goto action for state {current_state} and non-terminal '{lhs}'", "stack": stack[:], "buffer": tokens[position:]})
                return False, moves
            
            stack.append(int(parsing_table[goto_key]))
        else:
            moves.append({"error": f"Invalid action {action}", "stack": stack[:], "buffer": tokens[position:]})
            return False, moves
...
def index():
    if request.method == 'POST':
        try:
            ...
            return render_template('index.html', parsing_table_input=parsing_table_input,
                                   grammar_rules_input=grammar_rules_input,
                                   input_string=input_string, result=result, moves=moves)
